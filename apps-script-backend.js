...
const CONFIG = {
  ...
  EMAIL_TEMPLATES: {
    CUSTOMER: {
      ...
      DELIVERY_INFO: 'Teslimat esnasında kimlik belgenizi yanınızda bulundurmanızı hatırlatmak isteriz. Ayrıca, saatinizin bakım ve kullanım koşulları hakkında kapsamlı bilgilendirme yapılacağından, teslimat için yaklaşık 30 dakikalık bir süre ayırmanızı öneririz.',
...
function generateEmailTemplate(type, data) {
  ...
  // Customer için ekstra paragraflar
  const additionalContent = type === 'customer' ? `
    <p style="line-height: 1.8;">${config.DELIVERY_INFO}</p>
...
function generateCustomerICS(data) {
  ...
  const icsContent = [
    ...
    'ACTION:DISPLAY',
    `DESCRIPTION:${CONFIG.ICS_TEMPLATES.REMINDERS.ON_TIME} ${CONFIG.ICS_TEMPLATES.REMINDERS.BRING_ID}`,
    'END:VALARM',
    'END:VEVENT',
    'END:VCALENDAR'
  ].join('\r\n');
